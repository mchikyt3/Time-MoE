...
class TimeMoeClassificationHead(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # or you can use mean pooling: hidden_states.mean(dim=1)
...
class TimeMoeForSequenceClassification(TimeMoePreTrainedModel):
    ...
    def forward(
        ...
        """
        Args:
            input_ids: Time series input of shape [batch_size, seq_len, input_size]
            labels: Classification labels of shape [batch_size]
        """
        return_dict = (
            return_dict if return_dict is not None else self.config.use_return_dict
        )
...
class TimeMoeForTokenClassification(TimeMoePreTrainedModel):
    ...
    def forward(
        ...
        Args:
            input_ids: Time series input of shape [batch_size, seq_len, input_size]
            labels: Token classification labels of shape [batch_size, seq_len]
        """
        return_dict = (
            return_dict if return_dict is not None else self.config.use_return_dict
        )
